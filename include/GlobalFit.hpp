...
public:
  ...
  int nFitPars = 0, nFrozenPars = 0, nNuisancePars = 0, nTotPars = 0, nTotalPoints = 0;
};
...
// std::function<void(Int_t& npar, Double_t* grad, Double_t& chi2, Double_t* par, Int_t flag)> jesFitter_wrapper(std::map<TString, ShapeContainer*> shapes,
// std::map<TString, DataContainer*> my_data, std::map<TString, NuisanceContainer*> sources);
