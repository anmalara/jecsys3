...
template <class T> void Print(const T text, const std::string color) { std::cout << color << text << " "; }

template <class T> void Print(const T text) { std::cout << text << " "; }

template <class T> void PrintLine(const T text, const std::string color) {
  Print(text, color);
  std::cout << reset << std::endl;
}

template <class T> void PrintLine(const T text) {
  Print(text);
  std::cout << reset << std::endl;
}
...
template <typename T> int FindInVector(const std::vector<T> &vec, const T &el) {
  int index = -1;
...
template <typename T> bool FindInMap(const std::map<TString, T> &map, const TString &el) { return (map.find(el) != map.end()); }

#include <sstream>

template <typename T> std::string to_string_with_precision(const T value, const int n = 3) {
...
template <typename T> int closest(std::vector<T> const &vec, T value) {
