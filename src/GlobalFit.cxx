...
void GlobalFit::LoadSystematics(){
  ...
  for (auto [name, info]: nuisances_map) {
    ...
    if (FindInVector(input_hnames,appliesTo)<0) {
      if (debug) PrintLine("Skipping nuisance: "+name+" index: "+to_string(FindInVector(input_hnames,appliesTo)), yellow);
      continue;
    }
    TString hname = ReplaceDefault(info["hname"]);
    if (debug) PrintLoading("nuisance", name, hname);
    unique_ptr<TH1D> h;
    ...
    if (debug) {PrintLine("successfully", yellow); cout << *nuisances[name] << endl;};
  }
}
...
void GlobalFit::LoadShapes(){
  ...
  for (auto [name, info]: shapes_map) {
    ...
    if (FindInVector(types,appliesTo)<0 || FindInVector(shapes_allowed,TString(name).ReplaceAll("_"+appliesTo,""))<0) {
      if (debug) PrintLine("Skipping shape: "+name+" index: "+to_string(FindInVector(types,appliesTo))+" "+to_string(FindInVector(shapes_allowed,appliesTo)), yellow);
      continue;
    }
...
void GlobalFit::LoadReference(){
  for (auto [name, info]: reference_obj_map) {
    TString hname = ReplaceDefault(info["hname"]);
    if (debug) PrintLoading("reference", name, hname);
...
void GlobalFit::CleanGraphs(){
  ...
  for (auto [dt_name,dt]: my_data){
    if (dt->type()=="Resp") continue;
    int removed_points = 0;
    for (int i = 0; i < dt->input()->GetN(); ++i){
      if (dt->input()->GetX()[i]<=ptmin_pf) removed_points++;
      if (dt->input()->GetX()[i]>=ptmax_pf) removed_points++;
    }
...
void GlobalFit::SetupFitFunction(){
  ...
  if (debug) PrintLine("Loaded "+to_string(my_data.size())+" graphs", green);
  if (debug) PrintLine("Loaded "+to_string(shapes.size())+" shapes", green);
  if (debug) PrintLine("Loaded "+to_string(nuisances.size())+" nuisances", green);
...
void GlobalFit::DoGlobalFit(){
  ...
  if (penalizeFitPars) nFittedDataPoints -= nFitPars-nFrozenPars;
...
void GlobalFit::StoreFitOutput(){
  ...
  for (auto [name,dt]: my_data){
    ...
    for (int i = 0; i < output->GetN(); ++i) bins.push_back((int)output->GetX()[i]);
  ...
  for (int i = 0; i < bins.size() ; i++) {
    ...
    for (auto [name,dt]: my_data){
          ...
          if (x < ref->GetX()[0]) continue;
          if (x > ref->GetX()[ref->GetN()-1]) continue;
          int i_graph = -1;
    ...
    for (auto [dt_name,dt]: my_data){
      ...
      for (int bin = 0; bin < graph_input->GetN(); ++bin) {
        ...
        for (auto [nuisance_name,nuisance]: nuisances){
          if (dt_name!=nuisance->appliesTo()) continue;
          // if (FindInString("prob",nuisance_name.Data()) !=nuisance->appliesTo()) continue;
          // if (debug) PrintLine("adding1 "+nuisance_name+" to shift for "+ dt_name+"("+nuisance->appliesTo()+")", yellow);
          TH1D* hnuis = nuisance->hist(); assert(hnuis);
