        ...
        if drawLogo:
            ...
            if writeExtraText:
                ...
                if extraText2 != "":
                    latex.DrawLatex(
                        posX_,
                        posY_ - relExtraDY * cmsTextSize * t - relExtraDY * extraTextSize * t,
                        extraText2,
                    )
                if len(extraText3) != 0:
                    latex.SetTextSize(extraTextSize * t)
                    ...
                    for ind, tt in enumerate(extraText3):
                        latex.DrawLatex(
                            ...
                            - 0.004
                            - (relExtraDY * extraTextSize * t if len(extraText2) != 0 else 0)
...
def tdrDraw(h, opt, marker=rt.kFullCircle, msize=1.0, mcolor=rt.kBlack, lstyle=rt.kSolid, lwidth=1, lcolor=-1, fstyle=1001, fcolor=rt.kYellow + 1, alpha=-1):
...
def FixXAsisPartition(canv, shift=None, textsize=0.05, bins=[30, 100, 300, 1000, 3000]):
    ...
    if shift == None:
        YMin, YMax = (
            GettdrCanvasHist(canv).GetYaxis().GetXmin(),
            GettdrCanvasHist(canv).GetYaxis().GetXmax(),
        )
