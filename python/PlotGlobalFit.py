...
class PlotGlobalFit:
    ...
    def LoadInputs(self):
        ...
        self.functionforms = OrderedDict(
            [
                ("ecalm3", {"leg": "ECAL scale", "color": rt.kGreen + 2, "npar": 4, "func": "ecalm3"}),
                ("ecalGain1p3", {"leg": "ECAL gain 1", "color": rt.kCyan + 1, "npar": 4, "func": "ecalGain1p3"}),
                ("ecalGain6p3", {"leg": "ECAL gain 6", "color": rt.kRed + 1, "npar": 4, "func": "ecalGain6p3"}),
                ("ecalGain12p3", {"leg": "ECAL gain 12", "color": rt.kAzure - 2, "npar": 4, "func": "ecalGain12p3"}),
                ("hadHcalp3", {"leg": "HCAL scale", "color": rt.kViolet + 2, "npar": 4, "func": "hadHcalp3"}),
                ("hadHcalH097", {"leg": "HCAL extr. -3%", "color": rt.kOrange, "npar": 4, "func": "hadHcalH097"}),
                ("hadHcalH100", {"leg": "HCAL extr. +0%", "color": rt.kTeal + 2, "npar": 4, "func": "hadHcalH100"}),
        ...
        for shape, info in self.functionforms.items():
            for mode in ["input", "prefit", "postfit"]:
                for pf in ["", "_chf", "_nhf", "_nef"]:
                    s_name = shape + mode + pf
                    self.shapes[s_name] = self.inputfile.Get("shape_" + mode + "_" + info["func"] + pf)
        ...
        self.infos["herr"] = {
            ...
            "legType": "extra",
            "legStyle": "f",
            "plotinfo": {"opt": "e3", "fcolor": ROOT.kCyan + 1, "lcolor": ROOT.kCyan + 1, "msize": 0},
        }
        self.infos["hjesref"] = {
            "objName": "herr_ref",
            "legName": "herr_ref",
            "legType": "extra",
            "legStyle": "f",
            "plotinfo": {"opt": "e3", "fcolor": ROOT.kYellow + 1, "lcolor": ROOT.kYellow + 1, "msize": 0},
        }
        self.infos["jesFit"] = {
            "objName": "jesFit_Resp",
            "legName": "#chi^{2}/n.d.f.",
        ...
        self.infos["jesFit_down"] = {
            "objName": "jesFit_down_Resp",
            "legName": None,
            "legType": "",
            "legStyle": "l",
            "plotinfo": {
        ...
        self.infos["jesFit_up"] = {
            "objName": "jesFit_up_Resp",
            "legName": None,
            "legType": "",
            "legStyle": "l",
            "plotinfo": {
        ...
        for mode in ["mpf", "db"]:
            self.infos["zjet_" + mode] = {
                "objName": "Resp_zjet_" + mode,
                "legName": "Z+jet",
                "legType": mode,
                "legStyle": "lep",
                "plotinfo": {"opt": "Pz", "msize": 1.2, "marker": ROOT.kFullStar if mode != "db" else ROOT.kOpenStar, "mcolor": ROOT.kRed + 1},
            }
            self.infos["gjet_" + mode] = {
                "objName": "Resp_gamjet_" + mode,
                "legName": "#gamma+jet",
                "legType": mode,
                "legStyle": "lep",
                "plotinfo": {"opt": "Pz", "msize": 0.8, "marker": ROOT.kFullSquare if mode != "db" else ROOT.kOpenSquare, "mcolor": ROOT.kBlue + 1},
            }
            self.infos["hadw_" + mode] = {
                "objName": "Resp_hadw_" + mode,
                "legName": "W#rightarrow qq",
                "legType": mode,
                "legStyle": "lep",
                "plotinfo": {"opt": "Pz", "msize": 0.9, "marker": ROOT.kFullCircle if mode != "db" else ROOT.kOpenCircle, "mcolor": ROOT.kGreen + 2},
            }
            self.infos["multijet_" + mode] = {
                "objName": "Resp_multijet_" + mode,
                "legName": "Multijet (p_{T}^{" + ScaleText("leading") + "})",
                "legType": mode,
                "legStyle": "lep",
                "plotinfo": {"opt": "Pz", "msize": 0.9, "marker": ROOT.kFullTriangleUp if mode != "db" else ROOT.kOpenTriangleUp, "mcolor": ROOT.kBlack},
            }
            self.infos["multijet_recoil_" + mode] = {
                "objName": "Resp_multijet_recoil_" + mode,
                "legName": "Multijet (p_{T}^{" + ScaleText("recoil") + "})",
                "legType": mode,
                "legStyle": "lep",
                "plotinfo": {"opt": "Pz", "msize": 0.9, "marker": ROOT.kFullTriangleDown if mode != "db" else ROOT.kOpenTriangleDown, "mcolor": ROOT.kGray + 1},
            }
            self.infos["incljet_" + mode] = {
                "objName": "Resp_incljet_" + mode,
                "legName": "Incl. jet",
                "legType": mode,
                "legStyle": "lep",
                "plotinfo": {"opt": "Pz", "msize": 0.8, "marker": ROOT.kFullDiamond if mode != "db" else ROOT.kOpenDiamond, "mcolor": ROOT.kOrange + 2},
            }

        self.pf_style = OrderedDict(
            [("chf", (ROOT.kRed + 1, ROOT.kFullTriangleUp)), ("nhf", (ROOT.kGreen + 2, ROOT.kFullTriangleDown)), ("nef", (ROOT.kAzure + 2, ROOT.kFullSquare))]
        )
        self.infos_PF = OrderedDict()
        mode, color, marker = ("Resp", ROOT.kBlack, ROOT.kFullCircle)
        self.infos_PF[mode + "variation_output"] = {
            "objName": "jesFit_graph_" + mode,
            "legName": mode,
            "legStyle": "lep",
            "plotinfo": {"opt": "e3", "fcolor": color, "lcolor": color, "alpha": 0.5},
        }
        self.infos_PF[mode + "prefit"] = {
            "objName": mode + "_comb_shift",
            "legName": mode,
            "legStyle": "lep",
            "plotinfo": {"opt": "Pz", "mcolor": color, "marker": marker, "msize": 0.9},
        }
        self.infos_PF[mode + "postfit"] = {
            "objName": mode + "_comb",
            "legName": mode,
            "legStyle": "lep",
            "plotinfo": {"opt": "Pz", "mcolor": color, "marker": marker},
        }

        for mode, (color, marker) in self.pf_style.items():
            self.infos_PF[mode + "variation_input"] = {
                "objName": mode + "_variation_input",
                "legName": mode,
                "legStyle": "lep",
                "plotinfo": {"opt": "e3", "fcolor": color, "lcolor": ROOT.kGray, "alpha": 0.3},
            }
            self.infos_PF[mode + "variation_output"] = {
                "objName": mode + "_variation_output",
                "legName": mode,
                "legStyle": "lep",
                "plotinfo": {"opt": "e3", "fcolor": color, "lcolor": color, "alpha": 0.5},
            }
            self.infos_PF[mode + "prefit"] = {
                "objName": mode + "_prefit",
                "legName": mode,
                "legStyle": "lep",
                "plotinfo": {"opt": "Pz", "mcolor": color, "marker": marker},
            }
            self.infos_PF[mode + "postfit"] = {
                "objName": mode + "_postfit",
                "legName": mode,
                "legStyle": "lep",
                "plotinfo": {"opt": "Pz", "mcolor": color + 1, "marker": marker},
            }
    ...
    def PlotPFVariations(self):
        ...
        for pf in ["_chf", "_nhf", "_nef"]:
            ...
            for name, shape in self.shapes.items():
                if "input" in name or not pf in name:
                    continue
